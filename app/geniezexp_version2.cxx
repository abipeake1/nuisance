...
Eigen::MatrixXd GetPCAFromCorr(bool loud = false) {
  ...
  // Eigen returns the EVs in ascending order, flip them
  for (int i = 0; i < 4; ++i) {
    Eigenvalues[i] = eigensolver.eigenvalues()[3 - i];
    Eigenvectors.col(i) = eigensolver.eigenvectors().col(3 - i);
  }

  if (loud) {
    for (int i = 0; i < 4; ++i) {
      std::cout << "EVal[" << i << "]: " << Eigenvalues[i] << ", EVect[" << i
  ...
  for (size_t i = 0; i < 4; ++i) {
    Eigenvectors.col(i) *= std::sqrt(Eigenvalues[i]);
  }
...
Eigen::VectorXd GetAVals(std::vector<double> const &BVals,
  ...
  for (size_t i = 0; i < 4; ++i) {
    ...
    /*
     std::cout<<"in for loop in GetAVals function and this is loop" << i
     <<std::endl; std::cout<< "The bvalues are" << BVals[i] <<std::endl;
...
Eigen::Vector4d product =
    central_values_a_from_genie * central_values_afrom_errors_genie;
...
Eigen::Array4d ScaleAparamsforGenie(std::vector<double> const &BVals) {
  ...
  std::cout << " syst_avals......Systematic a values" << syst_avals
            << std::endl;
  Eigen::VectorXd avalues_for_genie =
      syst_avals.array() / central_values_afrom_errors_genie_code.array();
  return avalues_for_genie;
}

void SetSystematicInTermsOfB(std::vector<double> const &BVals,
                             GReWeightNuXSecCCQE &grw) {
  std::vector<GSyst_t> syst_dials = {
      kXSecTwkDial_ZExpA1CCQE, kXSecTwkDial_ZExpA2CCQE, kXSecTwkDial_ZExpA3CCQE,
      kXSecTwkDial_ZExpA4CCQE};
  Eigen::VectorXd syst_avals = GetAVals(BVals, GetPCAFromCorr());
  Eigen::VectorXd avalues_for_genie_old =
      syst_avals.array() / central_values_afrom_errors_genie_code.array();
  ...
  Eigen::Vector4d GRW_fZExpTwkDial_bef;
  Eigen::Vector4d GRW_fZExpDef_bef;
  Eigen::Vector4d GRW_fZExpCurr_bef;
  Eigen::Vector4d GRW_fracerr_bef;

  Eigen::Vector4d GRW_fZExpTwkDial_aft;
  Eigen::Vector4d GRW_fZExpDef_aft;
  Eigen::Vector4d GRW_fZExpCurr_aft;
  Eigen::Vector4d GRW_fracerr_aft;

  GSystUncertainty *fracerr = GSystUncertainty::Instance();

  for (int i = 0; i < grw.fZExpMaxCoef; ++i) {
    GRW_fZExpTwkDial_bef[i] = grw.fZExpTwkDial[i];
    GRW_fZExpDef_bef[i] = grw.fZExpDef[i];
    GRW_fZExpCurr_bef[i] = grw.fZExpCurr[i];
    GRW_fracerr_bef[i] =
        fracerr->OneSigmaErr(syst_dials[i], grw.fZExpTwkDial[i] > 0 ? 1 : -1);
  }

  grw.Reconfigure();

  fracerr = GSystUncertainty::Instance();

  for (int i = 0; i < grw.fZExpMaxCoef; ++i) {
    GRW_fZExpTwkDial_aft[i] = grw.fZExpTwkDial[i];
    GRW_fZExpDef_aft[i] = grw.fZExpDef[i];
    GRW_fZExpCurr_aft[i] = grw.fZExpCurr[i];
    GRW_fracerr_aft[i] =
        fracerr->OneSigmaErr(syst_dials[i], grw.fZExpTwkDial[i] > 0 ? 1 : -1);
  }

  std::cout << fmt::format("{:>{}}", "i", 3) << "|";
  std::cout << fmt::format("{:>{}}", "b", 4) << "|";
  std::cout << fmt::format("{:>{}}", "da", 7) << "|";
  std::cout << fmt::format("{:>{}}", "a", 7) << "|";
  std::cout << fmt::format("{:>{}}", "gerr", 7) << "|";
  std::cout << fmt::format("{:>{}}", "gao", 7) << "|";
  std::cout << fmt::format("{:>{}}", "ga", 7) << "|";
  std::cout << fmt::format("{:>{}}", "grw_twk", 7) << "|";
  std::cout << fmt::format("{:>{}}", "grw_cur", 7) << "|";
  std::cout << fmt::format("{:>{}}", "grw_fer", 7) << std::endl;
  std::cout << fmt::format("{:->{}}", "", 3 + 4 + (8 * 7)) << std::endl;

  for (int i = 0; i < 4; ++i) {
    std::cout << fmt::format("{:>{}}", i, 3) << "|";
    std::cout << fmt::format("{:>{}}", BVals[i], 4) << "|";
    std::cout << fmt::format("{:>{}.4}", syst_avals[i], 7) << "|";
    std::cout << fmt::format("{:>{}.4}",
                             PRD_93_113015::a_14_cv[i] + syst_avals[i], 7)
              << "|";
    std::cout << fmt::format("{:>{}.4}",
                             central_values_afrom_errors_genie_code[i], 7)
              << "|";
    std::cout << fmt::format("{:>{}.4}", avalues_for_genie_old[i], 7) << "|";
    std::cout << fmt::format("{:>{}.4}", avalues_for_genie[i], 7) << "|";
    std::cout << fmt::format("{:>{}.4}", GRW_fZExpTwkDial_aft[i], 7) << "|";
    std::cout << fmt::format("{:>{}.4}", GRW_fZExpCurr_aft[i], 7) << "|";
    std::cout << fmt::format("{:>{}.4}", GRW_fracerr_aft[i], 7) << std::endl;
  }
...
void lukeslazytest() {
  ...
  std::cout << "PRD_93_113015::a_14_cv\n"
            << PRD_93_113015::a_14_cv << std::endl;
  std::cout << "PRD_93_113015::a_14_errors\n"
            << PRD_93_113015::a_14_errors << std::endl;
  std::cout << "PRD_93_113015::a_14_correlation\n"
            << PRD_93_113015::a_14_correlation << std::endl;
  std::cout << "PRD_93_113015::a_14_cov\n"
            << PRD_93_113015::a_14_cov() << std::endl;
  std::cout << "Covariance_Matrix\n" << Covariance_Matrix << std::endl;

  Eigen::MatrixXd PCA = GetPCAFromCorr(true);
  std::cout << "PCA:" << std::endl;
  for (int i = 0; i < 4; ++i) {
    std::cout << "b[" << i << "] = " << PCA.col(i).transpose() << std::endl;
  }

  genie::rew::GReWeightNuXSecCCQE ccqerw;
  SetSystematicInTermsOfB({1, 0, 0, 0}, ccqerw);
}
...
int main(int argc, char const *argv[]) {
  ...
  // std::string Tune_name = "G18_02a_00_000"; //find Tune_name using echo
  // $GENIE_XSEC_TUNE
  ...
  TFile *file_out =
      TFile::Open("myoutput_test_new_10e5_2608_new_lowerbins.root", "RECREATE");
  TTree *tree_out = new TTree("mytree", "mytreetitle");

  // This is the re-weighting tool in GENIE
  genie::rew::GReWeightNuXSecCCQEaxial
      myaxreweighter; // probably dont need this anymore
  ...
  genie::rew::GReWeightNuXSecCCQE
      myccqereweighter_altogether; // reweight a1 a2 a3 a4 simul.
  ...
  genie::rew::GReWeightNuXSecCCQE
      myccqereweighter_altogether_minus; // reweight a1 a2 a3 a4 simul.
  ...
  SetSystematicInTermsOfB(
      {1, 1, 1, 1},
      myccqereweighter_rw_b1_b2_b3_b4); // shift the b params all by plus 1 std
  SetSystematicInTermsOfB(
      {-1, -1, -1, -1},
      myccqereweighter_rw_b1_b2_b3_b4_minus); // shift the b params all by minus
                                              // 1 std

  SetSystematicInTermsOfB({0.1, 0.1, 0.1, 0.1},
                          myccqereweighter_rw_b1_b2_b3_b4_point1);
  SetSystematicInTermsOfB({-0.1, -0.1, -0.1, -0.1},
                          myccqereweighter_rw_b1_b2_b3_b4_minus_point1);
  ...
  std::cout << myaxreweighter.IsHandled(kXSecTwkDial_AxFFCCQEshape)
            << "does the ccurrent weight calc. handle this param?" << std::endl;

  myccqereweighter.SetSystematic(kXSecTwkDial_ZExpA1CCQE,
                                 1); // shift a1 by plus 1 sigma
  ...
  myccqereweighter_altogether.SetSystematic(
      kXSecTwkDial_ZExpA1CCQE,
      1); // vary a1 a2 a3 a4 in same reweight calculation
  ...
  myccqereweighter_altogether_minus.SetSystematic(
      kXSecTwkDial_ZExpA1CCQE,
      -1); // vary a1 a2 a3 a4 in same reweight calculation
  ...
  TH1 *enu = new TH1D("enu", "Enu;E_{nu} (GeV);Count", 35, 0,
                      70); // new TH1 for energy of incoming neutrino

  TCanvas *c4 = new TCanvas("c4", "Number of Final state protons per event");
  TH1 *protonnumber = new TH1D("protonnumber", "Np;N_{p};Count", 20, 0,
                               40); // new TH1 for energy of incoming neutrino

  // new TH3 for momentum (px,py,pz) of highest momentum final state protons
  TCanvas *c3 = new TCanvas("c3", "Momentum of final proton");
  c3->Divide(3, 1);
  TH1 *protonmomentumx =
      new TH1D("protonmomentumx", "pproton;p_{px} (GeV/c);Count", 40, -5, 5);
  TH1 *protonmomentumy =
      new TH1D("protonmomentumy", "pproton;p_{py} (GeV/c);Count", 40, -5, 5);
  TH1 *protonmomentumz =
      new TH1D("protonmomentumz", "pproton;p_{pz} (GeV/c);Count", 40, -5, 5);

  TH1 *finalstateprotons_with_greatestmomentum =
      new TH1D("pfinalstateprotons_with_greatestmomentum",
               "pprotonmax;p_{pmax} (GeV/c);Count", 100, -0.5, 5);

  TH1 *proton_momentum_total = new TH1D(
      "protonmomentumtotal", "pproton;p_{magnitude} (GeV/c);Count", 100, -2, 5);
  ...
  TH1 *q2_hist_rw_altogether =
      new TH1D("Q2histrwaltogether", "q2;Q^{2};Count", 35, 0, 7);

  TH1 *q2_hist_rw_minus = new TH1D("Q2histrwminus", "q2;Q^{2};Count", 35, 0, 7);
  TH1 *q2_hist_rw_a1_minus =
      new TH1D("Q2histrwa1minus", "q2;Q^{2};Count", 35, 0, 7);
  TH1 *q2_hist_rw_a2_minus =
      new TH1D("Q2histrwa2minus", "q2;Q^{2};Count", 35, 0, 7);
  TH1 *q2_hist_rw_a3_minus =
      new TH1D("Q2histrwa3minus", "q2;Q^{2};Count", 35, 0, 7);
  TH1 *q2_hist_rw_a4_minus =
      new TH1D("Q2histrwa4minus", "q2;Q^{2};Count", 35, 0, 7);
  TH1 *q2_hist_rw_altogether_minus =
      new TH1D("Q2histrwaltogetherminus", "q2;Q^{2};Count", 35, 0, 7);

  TH1 *q2_hist_myccqereweighter_rw_b1 =
      new TH1D("Q2hist_myccqereweighter_rw_b1", "q2;Q^{2};Count", 35, 0, 7);
  TH1 *q2_hist_myccqereweighter_rw_b2 =
      new TH1D("Q2hist_myccqereweighter_rw_b2", "q2;Q^{2};Count", 35, 0, 7);
  TH1 *q2_hist_myccqereweighter_rw_b3 =
      new TH1D("Q2hist_myccqereweighter_rw_b3", "q2;Q^{2};Count", 35, 0, 7);
  TH1 *q2_hist_myccqereweighter_rw_b4 =
      new TH1D("Q2hist_myccqereweighter_rw_b4", "q2;Q^{2};Count", 35, 0, 7);

  TH1 *q2_hist_myccqereweighter_rw_b1_minus = new TH1D(
      "Q2hist_myccqereweighter_rw_b1_minus", "q2;Q^{2};Count", 35, 0, 7);
  TH1 *q2_hist_myccqereweighter_rw_b2_minus = new TH1D(
      "Q2hist_myccqereweighter_rw_b2__minus", "q2;Q^{2};Count", 35, 0, 7);
  TH1 *q2_hist_myccqereweighter_rw_b3_minus = new TH1D(
      "Q2hist_myccqereweighter_rw_b3_minus", "q2;Q^{2};Count", 35, 0, 7);
  TH1 *q2_hist_myccqereweighter_rw_b4_minus = new TH1D(
      "Q2hist_myccqereweighter_rw_b4_minus", "q2;Q^{2};Count", 35, 0, 7);

  TH1 *q2_hist_myccqereweighter_rw_b1_b2 =
      new TH1D("Q2hist_myccqereweighter_rw_b1_b2", "q2;Q^{2};Count", 35, 0, 7);
  TH1 *q2_hist_myccqereweighter_rw_b1_b2_minus = new TH1D(
      "Q2hist_myccqereweighter_rw_b1_b2_minus", "q2;Q^{2};Count", 35, 0, 7);

  TH1 *q2_hist_myccqereweighter_rw_b3_b4_minus = new TH1D(
      "Q2hist_myccqereweighter_rw_b3_b4_minus", "q2;Q^{2};Count", 35, 0, 7);

  TH1 *q2_hist_myccqereweighter_rw_b1_b3 =
      new TH1D("Q2hist_myccqereweighter_rw_b1_3", "q2;Q^{2};Count", 35, 0, 7);
  TH1 *q2_hist_myccqereweighter_rw_b1_b3_minus = new TH1D(
      "Q2hist_myccqereweighter_rw_b1_b3_minus", "q2;Q^{2};Count", 35, 0, 7);

  TH1 *q2_hist_myccqereweighter_rw_b2_b3 =
      new TH1D("Q2hist_myccqereweighter_rw_b2_b3", "q2;Q^{2};Count", 35, 0, 7);
  TH1 *q2_hist_myccqereweighter_rw_b2_b3_minus = new TH1D(
      "Q2hist_myccqereweighter_rw_b2_b3_minus", "q2;Q^{2};Count", 35, 0, 7);

  TH1 *q2_hist_myccqereweighter_rw_b1_b4 =
      new TH1D("Q2hist_myccqereweighter_rw_b1_b34", "q2;Q^{2};Count", 35, 0, 7);
  TH1 *q2_hist_myccqereweighter_rw_b1_b4_minus = new TH1D(
      "Q2hist_myccqereweighter_rw_b1_b4_minus", "q2;Q^{2};Count", 35, 0, 7);

  TH1 *q2_hist_myccqereweighter_rw_b2_b4 =
      new TH1D("Q2hist_myccqereweighter_rw_b2_b4", "q2;Q^{2};Count", 35, 0, 7);
  TH1 *q2_hist_myccqereweighter_rw_b2_b4_minus = new TH1D(
      "Q2hist_myccqereweighter_rw_b2_b4_minus", "q2;Q^{2};Count", 35, 0, 7);

  TH1 *q2_hist_myccqereweighter_rw_b3_b4 =
      new TH1D("Q2hist_myccqereweighter_rw_b3_b4", "q2;Q^{2};Count", 35, 0, 7);
  TH1 *q2_hist_myccqereweighter_rw_b3_b3_minus = new TH1D(
      "Q2hist_myccqereweighter_rw_b3_b4_minus", "q2;Q^{2};Count", 35, 0, 7);

  TH1 *q2_hist_myccqereweighter_rw_b1_b2_b3 = new TH1D(
      "Q2hist_myccqereweighter_rw_b1_b2_b3", "q2;Q^{2};Count", 35, 0, 7);
  TH1 *q2_hist_myccqereweighter_rw_b1_b2_b3_minus = new TH1D(
      "Q2hist_myccqereweighter_rw_b1_b2_b3_minus", "q2;Q^{2};Count", 35, 0, 7);

  TH1 *q2_hist_myccqereweighter_rw_b1_b2_b4 = new TH1D(
      "Q2hist_myccqereweighter_rw_b1_b2_b4", "q2;Q^{2};Count", 35, 0, 7);
  TH1 *q2_hist_myccqereweighter_rw_b1_b2_b4_minus = new TH1D(
      "Q2hist_myccqereweighter_rw_b1_b2_b4_minus", "q2;Q^{2};Count", 35, 0, 7);

  TH1 *q2_hist_myccqereweighter_rw_b2_b3_b4 = new TH1D(
      "Q2hist_myccqereweighter_rw_b2_b3_b4", "q2;Q^{2};Count", 35, 0, 7);
  TH1 *q2_hist_myccqereweighter_rw_b2_b3_b4_minus = new TH1D(
      "Q2hist_myccqereweighter_rw_b2_b3_b4_minus", "q2;Q^{2};Count", 35, 0, 7);

  TH1 *q2_hist_myccqereweighter_rw_b1_b3_b4 = new TH1D(
      "Q2hist_myccqereweighter_rw_b1_b3_b4", "q2;Q^{2};Count", 35, 0, 7);
  TH1 *q2_hist_myccqereweighter_rw_b1_b3_b4_minus = new TH1D(
      "Q2hist_myccqereweighter_rw_b1_b3_b4_minus", "q2;Q^{2};Count", 35, 0, 7);

  TH1 *q2_hist_myccqereweighter_rw_b1_b2_b3_b4 = new TH1D(
      "Q2hist_myccqereweighter_rw_b1_b2_b3_b4", "q2;Q^{2};Count", 35, 0, 7);
  TH1 *q2_hist_myccqereweighter_rw_b1_b2_b3_b4_minus =
      new TH1D("Q2hist_myccqereweighter_rw_b1_b2_b3_b4_minus", "q2;Q^{2};Count",
               35, 0, 7);

  TH1 *q2_hist_myccqereweighter_rw_b1_b2_b3_b4_point1 =
      new TH1D("Q2hist_myccqereweighter_rw_b1_b2_b3_b4_point1",
               "q2;Q^{2};Count", 35, 0, 7);
  TH1 *q2_hist_myccqereweighter_rw_b1_b2_b3_b4_minus_point1 =
      new TH1D("Q2hist_myccqereweighter_rw_b1_b2_b3_b4_minus_point1",
               "q2;Q^{2};Count", 35, 0, 7);
  ...
  std::vector<std::unique_ptr<GReWeightNuXSecCCQE>>
      rand_b_reweight_engines; // vector of reweighters
  for (int throw_i = 0; throw_i < NThrows; throw_i++) {

    rand_b_reweight_engines.push_back(
    ...
    for (int i = 0; i < 4; ++i) {
      ...
      // std::cout<<"random b value in loop" << i <<
      // "is"<<randbvals[i]<<std::endl;
    }
    ...
    std::cout << "Have pushed back randbvals now goingt to set systematic "
                 "interms of b-----"
              << std::endl;
    SetSystematicInTermsOfB(randbvals, *rand_b_reweight_engines.back());
    randbval_vector.push_back(randbvals);
    std::cout << "The length of the vector rand_b_reweight_engines is "
              << rand_b_reweight_engines.size() << std::endl;
    tree_out->Fill();
  }
  ...
  TFile *results =
      new TFile("10e5events_10GeV_test_new_2608_new_lowerbins.root", "RECREATE",
                "105events_10GeV_test_new");

  for (Long64_t e_it = 0; e_it < nevt; ++e_it) {
    // std::cout<< "in event loop with entry-----------------------------------
    // "<< e_it <<std::endl;
    tr->GetEntry(e_it);
    ...
    double weight2 =
        myccqereweighter.CalcWeight(*ntpl->event); // get event in object ntpl
    double weight_a2 = myccqereweighter_a2.CalcWeight(
        *ntpl->event); // get event in object ntpl
    double weight_a3 = myccqereweighter_a3.CalcWeight(
        *ntpl->event); // get event in object ntpl

    double weight_a4 = myccqereweighter_a4.CalcWeight(
        *ntpl->event); // get event in object ntpl
    double weight_altogether =
        myccqereweighter_altogether.CalcWeight(*ntpl->event);

    // double weight_minus = myaxreweighter_minus.CalcWeight(*ntpl-> event);
    double weight2_minus = myccqereweighter_minus.CalcWeight(
        *ntpl->event); // get event in object ntpl
    double weight_a2_minus = myccqereweighter_a2_minus.CalcWeight(
        *ntpl->event); // get event in object ntpl
    double weight_a3_minus = myccqereweighter_a3_minus.CalcWeight(
        *ntpl->event); // get event in object ntpl
    double weight_a4_minus = myccqereweighter_a4_minus.CalcWeight(
        *ntpl->event); // get event in object ntpl
    double weight_altogether_minus =
        myccqereweighter_altogether_minus.CalcWeight(*ntpl->event);
    double myccqereweighter_a_newbasis_plus =
        myccqereweighter_altogether_minus.CalcWeight(*ntpl->event);

    double weight_myccqereweighter_rw_b1 =
        myccqereweighter_rw_b1.CalcWeight(*ntpl->event);
    double weight_myccqereweighter_rw_b2 =
        myccqereweighter_rw_b2.CalcWeight(*ntpl->event);
    double weight_myccqereweighter_rw_b3 =
        myccqereweighter_rw_b3.CalcWeight(*ntpl->event);
    double weight_myccqereweighter_rw_b4 =
        myccqereweighter_rw_b4.CalcWeight(*ntpl->event);

    double weight_myccqereweighter_rw_b1_minus =
        myccqereweighter_rw_b1_minus.CalcWeight(*ntpl->event);
    double weight_myccqereweighter_rw_b2_minus =
        myccqereweighter_rw_b2_minus.CalcWeight(*ntpl->event);
    double weight_myccqereweighter_rw_b3_minus =
        myccqereweighter_rw_b3_minus.CalcWeight(*ntpl->event);
    double weight_myccqereweighter_rw_b4_minus =
        myccqereweighter_rw_b4_minus.CalcWeight(*ntpl->event);

    double weight_myccqereweighter_rw_b1_b2 =
        myccqereweighter_rw_b1_b2.CalcWeight(*ntpl->event);
    double weight_myccqereweighter_rw_b2_b3 =
        myccqereweighter_rw_b2_b3.CalcWeight(*ntpl->event);
    double weight_myccqereweighter_rw_b1_b3 =
        myccqereweighter_rw_b1_b3.CalcWeight(*ntpl->event);
    double weight_myccqereweighter_rw_b1_b4 =
        myccqereweighter_rw_b1_b4.CalcWeight(*ntpl->event);
    double weight_myccqereweighter_rw_b2_b4 =
        myccqereweighter_rw_b2_b4.CalcWeight(*ntpl->event);
    double weight_myccqereweighter_rw_b3_b4 =
        myccqereweighter_rw_b3_b4.CalcWeight(*ntpl->event);

    double weight_myccqereweighter_rw_b1_b4_minus =
        myccqereweighter_rw_b1_b4_minus.CalcWeight(*ntpl->event);
    double weight_myccqereweighter_rw_b2_b4_minus =
        myccqereweighter_rw_b2_b4_minus.CalcWeight(*ntpl->event);
    double weight_myccqereweighter_rw_b1_b2_minus =
        myccqereweighter_rw_b1_b2_minus.CalcWeight(*ntpl->event);
    double weight_myccqereweighter_rw_b1_b3_minus =
        myccqereweighter_rw_b1_b3_minus.CalcWeight(*ntpl->event);
    double weight_myccqereweighter_rw_b2_b3_minus =
        myccqereweighter_rw_b2_b3_minus.CalcWeight(*ntpl->event);
    double weight_myccqereweighter_rw_b3_b4_minus =
        myccqereweighter_rw_b3_b4_minus.CalcWeight(*ntpl->event);

    double weight_myccqereweighter_rw_b1_b2_b3 =
        myccqereweighter_rw_b1_b2_b3.CalcWeight(*ntpl->event);
    double weight_myccqereweighter_rw_b1_b2_b3_minus =
        myccqereweighter_rw_b1_b2_b3_minus.CalcWeight(*ntpl->event);

    double weight_myccqereweighter_rw_b1_b2_b4 =
        myccqereweighter_rw_b1_b2_b4.CalcWeight(*ntpl->event);
    double weight_myccqereweighter_rw_b1_b2_b4_minus =
        myccqereweighter_rw_b1_b2_b4_minus.CalcWeight(*ntpl->event);

    double weight_myccqereweighter_rw_b2_b3_b4 =
        myccqereweighter_rw_b2_b3_b4.CalcWeight(*ntpl->event);
    double weight_myccqereweighter_rw_b2_b3_b4_minus =
        myccqereweighter_rw_b2_b3_b4_minus.CalcWeight(*ntpl->event);

    double weight_myccqereweighter_rw_b1_b3_b4 =
        myccqereweighter_rw_b1_b3_b4.CalcWeight(*ntpl->event);
    double weight_myccqereweighter_rw_b1_b3_b4_minus =
        myccqereweighter_rw_b1_b3_b4_minus.CalcWeight(*ntpl->event);

    double weight_myccqereweighter_rw_b1_b2_b3_b4 =
        myccqereweighter_rw_b1_b2_b3_b4.CalcWeight(*ntpl->event);
    double weight_myccqereweighter_rw_b1_b2_b3_b4_minus =
        myccqereweighter_rw_b1_b2_b3_b4_minus.CalcWeight(*ntpl->event);

    double weight_myccqereweighter_rw_b1_b2_b3_b4_point1 =
        myccqereweighter_rw_b1_b2_b3_b4_point1.CalcWeight(*ntpl->event);
    double weight_myccqereweighter_rw_b1_b2_b3_b4_minus_point1 =
        myccqereweighter_rw_b1_b2_b3_b4_minus_point1.CalcWeight(*ntpl->event);
    ...
    while ((p = (dynamic_cast<GHepParticle *>((iter).Next())))) {
      ...
      if ((p->Pdg() == 14) &&
          (p->Status() == 0)) { // if particle incoming neutrino muon
        // std::cout << "d incoming neutrino has energy " << p->E() <<
        // std::endl;
        enu->Fill(p->E());
      ...
      if ((p->Pdg() == 2212) && (p->Status() == 1)) {
        ...
        // std::cout << "d proton momentum is " << p->Px() << p->Py() << p->Pz()
        // << std::endl;
        proton_count++;
        ...
        // std::cout << "magnitude of proton momentum is" <<
        // proton_3_momentum_mag << "----------------------------" <<std::endl;
      ...
      final_state_protons_momenta_entry.push_back(
          proton_3_momentum_magnitude_entry);
    }
    ...
    ///// Working out which of the final state protons has the greatest momentum
    ///(magnitude)
    auto const max_iter =
        std::max_element(proton_3_momentum_magnitude_entry.begin(),
                         proton_3_momentum_magnitude_entry.end());
    ...
    q2_hist_myccqereweighter_rw_b1_minus->Fill(
        s2, weight_myccqereweighter_rw_b1_minus);
    q2_hist_myccqereweighter_rw_b2_minus->Fill(
        s2, weight_myccqereweighter_rw_b2_minus);
    q2_hist_myccqereweighter_rw_b3_minus->Fill(
        s2, weight_myccqereweighter_rw_b3_minus);
    q2_hist_myccqereweighter_rw_b4_minus->Fill(
        s2, weight_myccqereweighter_rw_b4_minus);

    q2_hist_myccqereweighter_rw_b1_b2->Fill(s2,
                                            weight_myccqereweighter_rw_b1_b2);
    q2_hist_myccqereweighter_rw_b1_b3->Fill(s2,
                                            weight_myccqereweighter_rw_b1_b3);
    q2_hist_myccqereweighter_rw_b2_b3->Fill(s2,
                                            weight_myccqereweighter_rw_b2_b3);
    q2_hist_myccqereweighter_rw_b1_b4->Fill(s2,
                                            weight_myccqereweighter_rw_b1_b4);
    q2_hist_myccqereweighter_rw_b2_b4->Fill(s2,
                                            weight_myccqereweighter_rw_b2_b4);
    q2_hist_myccqereweighter_rw_b3_b4->Fill(s2,
                                            weight_myccqereweighter_rw_b3_b4);

    q2_hist_myccqereweighter_rw_b1_b2_b3->Fill(
        s2, weight_myccqereweighter_rw_b1_b2_b3);
    q2_hist_myccqereweighter_rw_b1_b3_b4->Fill(
        s2, weight_myccqereweighter_rw_b1_b3_b4);
    q2_hist_myccqereweighter_rw_b2_b3_b4->Fill(
        s2, weight_myccqereweighter_rw_b2_b3_b4);
    q2_hist_myccqereweighter_rw_b1_b2_b4->Fill(
        s2, weight_myccqereweighter_rw_b1_b2_b4);

    q2_hist_myccqereweighter_rw_b1_b2_minus->Fill(
        s2, weight_myccqereweighter_rw_b1_b2_minus);
    q2_hist_myccqereweighter_rw_b1_b3_minus->Fill(
        s2, weight_myccqereweighter_rw_b1_b3_minus);
    q2_hist_myccqereweighter_rw_b2_b3_minus->Fill(
        s2, weight_myccqereweighter_rw_b2_b3_minus);
    q2_hist_myccqereweighter_rw_b1_b4_minus->Fill(
        s2, weight_myccqereweighter_rw_b1_b4_minus);
    q2_hist_myccqereweighter_rw_b2_b4_minus->Fill(
        s2, weight_myccqereweighter_rw_b2_b4_minus);
    q2_hist_myccqereweighter_rw_b3_b4_minus->Fill(
        s2, weight_myccqereweighter_rw_b3_b4_minus);

    q2_hist_myccqereweighter_rw_b1_b2_b3_minus->Fill(
        s2, weight_myccqereweighter_rw_b1_b2_b3_minus);
    q2_hist_myccqereweighter_rw_b1_b3_b4_minus->Fill(
        s2, weight_myccqereweighter_rw_b1_b3_b4_minus);
    q2_hist_myccqereweighter_rw_b2_b3_b4_minus->Fill(
        s2, weight_myccqereweighter_rw_b2_b3_b4_minus);
    q2_hist_myccqereweighter_rw_b1_b2_b4_minus->Fill(
        s2, weight_myccqereweighter_rw_b1_b2_b4_minus);

    q2_hist_myccqereweighter_rw_b1_b2_b3_b4->Fill(
        s2, weight_myccqereweighter_rw_b1_b2_b3_b4);
    q2_hist_myccqereweighter_rw_b1_b2_b3_b4_minus->Fill(
        s2, weight_myccqereweighter_rw_b1_b2_b3_b4_minus);

    q2_hist_myccqereweighter_rw_b1_b2_b3_b4_point1->Fill(
        s2, weight_myccqereweighter_rw_b1_b2_b3_b4_point1);
    q2_hist_myccqereweighter_rw_b1_b2_b3_b4_minus_point1->Fill(
        s2, weight_myccqereweighter_rw_b1_b2_b3_b4_minus_point1);
    // loop through throws
  ...
  finalstateprotons_with_greatestmomentum->SetYTitle(
      "Highest momentum final state proton events");
  ...
  for (int bin_i = 0; bin_i < myhists.front()->GetXaxis()->GetNbins();
       ++bin_i) {
  ...
  std::cout << "avalues_for_genie_vector---------------------------------------"
               "---------------"
            << std::endl;
